...
# decky-proton-pulse (Node 20, Python 3.11, uv, pnpm).
#
...
BREW_NODE_DIR="/home/linuxbrew/.linuxbrew/opt/node@20/bin"
...
ensure_brew_node() {
  if [[ -x "$BREW_NODE_DIR/node" ]]; then
    echo "Linuxbrew node@20 already installed: $("$BREW_NODE_DIR/node" --version)"
    return
  fi
  ...
  eval "$(/home/linuxbrew/.linuxbrew/bin/brew shellenv)" && brew install node@20
}
...
mise_install() {
  ...
  echo "mise install failed — checking if node 20 needs a brew fallback ..."
  ...
  if ! "$mise_bin" where node >/dev/null 2>&1; then
    echo "mise could not install node 20. Falling back to Linuxbrew."
    ensure_brew_node
  fi
}
