...
update_changelog_for_release() {
  node scripts/sync-changelog.mjs --prepare-release "${RELEASE_TAG}"
}
...
confirm_release_actions() {
  ...
  if [[ -n "$GH_RELEASE" ]]; then
    if is_truthy "$DRY_RUN"; then
      echo "  - Would refresh ## Unreleased from unpushed commits when present"
      ...
      echo "  - Refresh ## Unreleased from unpushed commits when present"
