...
for each game with exponential backoff on transient failures.
"""
...
# GitHub Pages CDN hosting the per-game ProtonDB report data
...
def _parse_acf(path: Path) -> dict[str, str]:
    """Minimal ACF/VDF key-value parser -- returns top-level string pairs."""
    result: dict[str, str] = {}
    try:
        # ACF files are Valve's KeyValues format; errors="replace" handles
        # occasional non-UTF-8 bytes in game names
        text = path.read_text(errors="replace")
    except OSError:
        return result
    for m in re.finditer(r'"(\w+)"\s+"([^"]*)"', text):
        # Only captures top-level "key" "value" pairs; nested sections
        # (like "UserConfig") are ignored since we only need appid/name/size
        result[m.group(1)] = m.group(2)
    return result


def discover_installed_games() -> list[dict[str, Any]]:
    """Return installed games sorted by playtime descending.

    Each entry: ``{"app_id": str, "name": str, "playtime": int}``.
    ``playtime`` is ``SizeOnDisk`` as a rough proxy when real playtime
    isn't available from the manifest (higher = more invested).
    """
    ...
    for lib in _library_folders():
        ...
        for manifest in steamapps.glob("appmanifest_*.acf"):
            ...
            if not app_id or not app_id.isdigit():
                continue
            name = kv.get("name", "")
            # SizeOnDisk is a rough proxy for playtime -- real playtime lives
            # in localconfig.vdf which requires per-user parsing. Size is
            # always present in the manifest and correlates well enough.
    ...
    # Sort: most recently updated first, then by size as tiebreaker
...
def _parse_resp_headers(header_block: str) -> dict[str, str]:
    """Parse raw HTTP header block into a lowercase-keyed dict."""
...
def _curl_fetch(
    ...
    """Fetch a URL via curl, returning (status_code, body, response_headers).

    Returns status 0 on network error.
    """
    cmd = [
        "curl", "-LsS", "--http1.1",
        "--connect-timeout", "10",
    ...
    # curl -D - writes headers to stdout before body, -w appends status code.
    # So stdout looks like: <headers>\r\n\r\n<body>\n<http_code>
    raw = result.stdout
    ...
    # -w "\n%{http_code}" puts the status code on the very last line
...
def _fetch_cdn_json(url: str, app_id: str, filename: str) -> Any | None:
    """Fetch a CDN JSON file with conditional request + backoff retry.

    Returns parsed JSON on success, None on failure.  Writes to cache
    on 200, preserves existing cache on 304.
    """
    if is_fresh(url):
    ...
    for attempt in range(MAX_RETRIES):
        ...
        if status == 304:
            # Not modified -- refresh the timestamp
        ...
        if status == 404:
            ...
            return None  # game not in CDN, don't retry
...
def prefetch_installed_games(cancel_check: Any | None = None) -> dict[str, Any]:
    ...
    *cancel_check* is a ``threading.Event`` -- if set, abort early.
