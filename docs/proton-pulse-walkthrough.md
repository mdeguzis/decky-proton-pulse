...
- `config/ui_screenshot_manifest.json` -- defines 13 capture targets
...
```bash
make setup    # install pnpm + Python deps
...
`.github/workflows/autobuild.yml` runs on every push/PR: installs deps,
runs `make build`.
