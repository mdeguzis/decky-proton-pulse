...
- Config editor action bar reshuffled: Save (persist + push to cloud), Submit Report (opens report flow), Custom Toggle. Cancel removed since B on the gamepad already closes the modal
- Applying a config always replaces existing launch options now, no Append / Replace / Cancel prompt
- Proton kind list centralised in one module so Create Config and Submit Report share the same options (Valve Proton, Proton-GE, Proton-CachyOS, Native, Not Listed)
- Submit Report Proton version field is a dropdown of installed versions with a free-text fallback
- Submit Report action bar: three buttons in one row, Save Draft and Close now share the same style as Submit
- Verdict and out-of-box questions auto-advance to the next field on answer
- Validation summary error unified to "Please fill out this required field for submission."
- Delete Config confirmation warns that synced config and any submitted report are also removed from the cloud
- Non-Steam headers now use a shared GameBanner with Steam CDN then local grid artwork fallback, killing the purple broken-image glyph on Heroic / Junk Store shortcuts
- Header pill and subtitle show the actual launcher (Heroic / Junk Store) plus the underlying store (Epic Games / GOG) when known, sourced from the shortcut's launch options
- Steam game-page badge palette tuned: Heroic cyan-blue, Junk Store orange, Epic Games charcoal, GOG purple
- Junk Store shortcuts are now detected and their Epic / GOG store is surfaced from the launcher script path
- GOG installs on Deck are detected without requiring the Windows Galaxy launcher in the path
