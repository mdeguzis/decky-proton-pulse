...
- Config manager row shows Draft / Pending Approval / Published so users see when the auto-moderator has taken over (#11)
- Report modal shows the actual tracked playtime ("2.3 hr, auto-detected") instead of hiding the field when auto-populated (#12)
- Compat tool settings redesigned: per-row Info modal (install location, date, size), friendly name in the header, active version as subtitle, and versioned rows hidden when covered by a rolling slot (matches Valve UX)
- Game library badge no longer traps focus on right-arrow -- the nav mesh dead-end now escapes to the search bar automatically (#119)
- Scoring: normalize rating case at every lookup site so ProtonDB CDN's Capitalized ratings score correctly (#427 parity)
- Security policy: vulnerability disclosure written up in SECURITY.md + linked from the README (#110)
- Fixes: non-Steam boxart, self-report vote guard, arch guard on Proton-GE install, pulse vote auto-refresh, report modal UX polish, toast wrapping on Deck, rolling-slot naming in Steam picker, install-progress dedup on -Latest row, Release Notes menu clearance
