...
import { SystemRequirementsCompareModal } from '../components/ReportDetailModal';
...
// Remove stale injected items to prevent duplicates on re-renders.
const removeDupe = (items: any[]): void => {
  for (const key of ['proton-pulse-configure', 'proton-pulse-sysreq']) {
    const idx = items.findIndex((x: any) => x?.key === key);
...
const injectMenuItem = (
  ...
  const focusedAppIdAtInject = pageState.focusedAppId ?? 0;
  const menuItems: any[] = [
    <MenuItem
  ...
  if (focusedAppIdAtInject) {
    const focusedAppName = pageState.focusedAppName ?? '';
    menuItems.push(
      <MenuItem
        key="proton-pulse-sysreq"
        onSelected={() => {
          showModal(
            <SystemRequirementsCompareModal
              appId={focusedAppIdAtInject}
  ...
  items.splice(insertAt, 0, ...menuItems);
};
