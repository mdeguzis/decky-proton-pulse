...
async function runAutomationAction(action: ScreenshotAutomationAction): Promise<{
  ...
  if (action.steamRoute) {
    void logFrontendEvent('DEBUG', 'Screenshot automation navigating to Steam route', {
      steamRoute: action.steamRoute,
    });
    try {
      Navigation.Navigate(action.steamRoute);
    } catch {
