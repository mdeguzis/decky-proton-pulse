...
const mockGetUserConfigs = getUserConfigs as ReturnType<typeof vi.fn>;
...
describe('getProtonDBReports', () => {
  ...
  it('returns live Pulse reports from Supabase when CDN index 404s and ProtonDB has nothing', async () => {
    mockGetUserConfigs.mockResolvedValueOnce([{
      id: 99, client_id: 'x', app_id: '4291930', title: 'My Demo',
      cpu: 'AMD Custom APU', gpu: 'AMD Radeon', gpu_driver: 'Mesa 26', gpu_vendor: 'amd',
      ram: '16 GB', os: 'SteamOS 3.6', kernel: '6.16', proton_version: 'GE-Proton10-1',
      duration: 'underOneHour', rating: 'gold', notes: 'Runs great',
      launch_options: '', enabled_vars: {}, confidence_score: null, source: 'user',
      created_at: '2026-04-26T00:00:00.000Z',
    }]);
    // CDN miss, ProtonDB counts ok but no detailed reports, summary 404
    mockFetch
      .mockResolvedValueOnce(makeResponse(404, null))
      .mockResolvedValueOnce(makeResponse(200, { reports: 0, timestamp: 1775704890 }))
      .mockResolvedValueOnce(makeResponse(404, null))
      .mockResolvedValueOnce(makeResponse(404, null))
      .mockResolvedValueOnce(makeResponse(404, null));
    const result = await getProtonDBReportsWithDiagnostics('4291930');
    expect(result.diagnostics.source).toBe('pulse-live');
    expect(result.reports).toHaveLength(1);
    expect(result.reports[0]).toMatchObject({ appId: '4291930', rating: 'gold', protonVersion: 'GE-Proton10-1' });
    mockGetUserConfigs.mockResolvedValue([]);
  });
