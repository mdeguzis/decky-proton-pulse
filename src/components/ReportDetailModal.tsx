        ...
        <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: '12px 16px 60px' }}>
          ...
          >
            ...
            {report.gpuArchitecture ? (
              <InfoCompareRow label="GPU Arch" left={report.gpuArchitecture} right="-" match={null} />
            ) : null}
...
interface SysReqCompareRow {
  label: string;
  gameReq: string;
  ourValue: string;
  status: ReqFieldStatus;
}

const SYS_REQ_SCROLL_STEP = 80;

function parseVramMbFromString(str: string): number | null {
...
function numericFieldStatus(reqVal: number | null, sysVal: number | null): ReqFieldStatus {
  if (reqVal == null || sysVal == null) return 'unknown';
  if (sysVal >= reqVal) return 'pass';
  if (sysVal >= reqVal * 0.75) return 'maybe';
  return 'fail';
}

function buildSysReqRows(reqs: GameReqResponse | null, sysInfo: SystemInfo | null): SysReqCompareRow[] {
  if (!reqs?.fields?.length) return [];
  return reqs.fields.map((f) => {
    const label = f.label.toLowerCase();
    let ourValue = '-';
    let status: ReqFieldStatus = 'unknown';

    if (label.includes('memory') || label === 'ram') {
      ourValue = sysInfo?.ram_gb != null ? `${sysInfo.ram_gb} GB RAM` : '-';
      status = numericFieldStatus(reqs.min_ram_gb, sysInfo?.ram_gb ?? null);
    } else if (label === 'vram' || (label.includes('video') && label.includes('mem'))) {
      const reqMb = parseVramMbFromString(f.value);
      const sysGb = sysInfo?.vram_mb != null ? (sysInfo.vram_mb / 1024).toFixed(1) : null;
      ourValue = sysGb != null ? `${sysGb} GB` : '-';
      status = numericFieldStatus(reqMb, sysInfo?.vram_mb ?? null);
    } else if (label.includes('processor') || label === 'cpu') {
      ourValue = sysInfo?.cpu ?? '-';
    } else if (label.includes('graphic') || label.includes('video card') || label === 'gpu') {
      const sysGpu = sysInfo?.gpu ?? null;
      const sysVramGb = sysInfo?.vram_mb != null ? `${(sysInfo.vram_mb / 1024).toFixed(0)} GB VRAM` : null;
      ourValue = [sysGpu, sysVramGb].filter(Boolean).join(', ') || '-';
      const reqVramMb = parseVramMbFromString(f.value);
      if (reqVramMb != null && sysInfo?.vram_mb != null) {
        status = numericFieldStatus(reqVramMb, sysInfo.vram_mb);
      }
    } else if (label === 'os' || label.includes('operating system')) {
      ourValue = sysInfo?.distro ?? '-';
    } else if (label.includes('kernel')) {
      ourValue = sysInfo?.kernel ?? '-';
    }

    return { label: f.label, gameReq: f.value, ourValue, status };
  });
}

function computeSysReqVerdict(rows: SysReqCompareRow[]): 'pass' | 'fail' | 'maybe' | 'unknown' {
  const checkable = rows.filter((r) => r.status !== 'unknown');
  if (checkable.length === 0) return 'unknown';
  if (checkable.some((r) => r.status === 'fail')) return 'fail';
  if (checkable.some((r) => r.status === 'maybe')) return 'maybe';
  return 'pass';
}

function SysReqRow({ row }: { row: SysReqCompareRow }) {
  const statusIcon = row.status === 'pass' ? '✓'
    : row.status === 'fail' ? '✗'
    : row.status === 'maybe' ? '~'
    : '?';
  const statusColor = row.status === 'pass' ? '#4caf50'
    : row.status === 'fail' ? '#f44336'
    : row.status === 'maybe' ? '#f59e0b'
    : '#555';
  return (
    <>
      <div style={{ padding: '10px 12px', borderTop: '1px solid rgba(255,255,255,0.06)', color: '#9db0c4', fontSize: 12 }}>
        {row.label}
      </div>
      <div style={{ padding: '10px 12px', borderTop: '1px solid rgba(255,255,255,0.06)', borderLeft: '1px solid rgba(255,255,255,0.04)', color: '#e8f4ff', fontSize: 12, wordBreak: 'break-word' }}>
        {row.gameReq}
      </div>
      <div style={{ padding: '10px 12px', borderTop: '1px solid rgba(255,255,255,0.06)', borderLeft: '1px solid rgba(255,255,255,0.04)', color: '#e8f4ff', fontSize: 12, wordBreak: 'break-word' }}>
        {row.ourValue}
      </div>
      <div style={{ padding: '10px 12px', borderTop: '1px solid rgba(255,255,255,0.06)', borderLeft: '1px solid rgba(255,255,255,0.04)', textAlign: 'center', fontSize: 16, fontWeight: 700, color: statusColor }}>
        {statusIcon}
      </div>
    </>
  );
...
export function SystemRequirementsCompareModal({
  closeModal,
  appId,
  appName: _appName,
}: {
  ...
  useEffect(() => {
    Promise.all([
      getSystemInfoSafe().then((info) => { if (info) setSysInfo(info); }).catch(() => {}),
      getGameRequirements(String(appId)).then(setReqs).catch(() => {}),
    ]).finally(() => setLoading(false));
  }, [appId]);

  const rows = buildSysReqRows(reqs, sysInfo);
  const verdict = computeSysReqVerdict(rows);
  const verdictBadge = verdict === 'pass'
    ? { text: t().detail.verdictPass, bg: '#2f6f4f', color: '#ecfff1' }
    : verdict === 'fail'
    ? { text: t().detail.verdictFail, bg: '#7a2f34', color: '#ffe8ea' }
    : verdict === 'maybe'
    ? { text: t().detail.verdictMaybe, bg: '#7d6123', color: '#fff5dc' }
    : null;
  ...
  return (
    <ModalRoot
      onCancel={closeModal}
      bAllowFullSize
      className="proton-pulse-sysreq-modal"
      modalClassName="proton-pulse-sysreq-modal"
    >
      <style>{`
        .proton-pulse-sysreq-modal,
        .proton-pulse-sysreq-modal > div,
        .proton-pulse-sysreq-modal .DialogContent_InnerWidth {
          padding: 0 !important; margin: 0 !important;
          max-width: 100vw !important; width: 100vw !important;
          max-height: 100vh !important;
        }
        .proton-pulse-sysreq-modal .ModalPosition { inset: 0 !important; }
      `}</style>
      <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 40px)' }}>
        <div style={{ flexShrink: 0, padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, borderBottom: '1px solid #2a3a4a' }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: '#e8f4ff' }}>
            {t().detail.systemRequirements}
          </div>
          {verdictBadge && (
            <div style={{ borderRadius: 999, padding: '4px 10px', fontSize: 11, fontWeight: 700, whiteSpace: 'nowrap', background: verdictBadge.bg, color: verdictBadge.color }}>
              {verdictBadge.text}
            </div>
          )}
        </div>
        <Focusable
          onGamepadDirection={(evt: GamepadEvent) => {
            ...
            if (!el) return;
            if (btn === GamepadButton.DIR_UP) {
              el.scrollBy({ top: el.scrollTop <= SYS_REQ_SCROLL_STEP ? -el.scrollTop : -SYS_REQ_SCROLL_STEP, behavior: 'auto' });
            } else if (btn === GamepadButton.DIR_DOWN) {
              evt.preventDefault();
              el.scrollBy({ top: SYS_REQ_SCROLL_STEP, behavior: 'auto' });
            }
          }}
          style={{ flexShrink: 0, display: 'flex', gap: 6, padding: '6px 12px', borderBottom: '1px solid #2a3a4a' }}
        >
          <DialogButton onClick={closeModal} style={{ flex: 1, fontSize: 10, padding: '5px 4px', minHeight: 0, minWidth: 0 }}>
            {t().common.close}
          </DialogButton>
        </Focusable>
        <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: '12px 16px 60px' }}>
          {loading ? (
            ...
            <div style={{ color: '#9db0c4', fontSize: 12, padding: '8px 0' }}>
              {t().detail.noGameRequirementsFound}
            </div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: '100px minmax(0,1fr) minmax(0,1fr) 44px', gap: 0, border: '1px solid #2a3a4a', borderRadius: 8, overflow: 'hidden', background: 'rgba(13,19,28,0.96)', width: '100%', boxSizing: 'border-box' }}>
              <div style={{ padding: '10px 12px', background: '#162333' }} />
              <div style={{ padding: '10px 12px', background: '#162333', color: '#e8f4ff', fontWeight: 700, fontSize: 12 }}>
                {t().detail.gameMinimum}
              </div>
              <div style={{ padding: '10px 12px', background: '#162333', color: '#e8f4ff', fontWeight: 700, fontSize: 12 }}>
                {t().detail.ourSystem}
              </div>
              <div style={{ padding: '10px 12px', background: '#162333', color: '#e8f4ff', fontWeight: 700, fontSize: 11, textAlign: 'center' }}>
                {t().detail.sysReqStatus}
              </div>
              {rows.map((row, i) => (
                <SysReqRow key={i} row={row} />
              ))}
            </div>
          )}
        </div>
...
export function ReportDetailModal({
  ...
  const handleEditConfig = () => {
    void logFrontendEvent('INFO', 'ReportDetail: Opening config editor pre-filled from report', {
      appId, appName, protonVersion: report.protonVersion,
    });
    // Parse the current launch options to carry over any existing toggles/vars,
    // then override the proton version with the report's version.
    const existing = parseLaunchOptions(currentLaunchOptions);
    const prefilledLaunchOptions = buildLaunchOptions(
      report.protonVersion || existing.protonVersion,
      existing.vars,
      existing.rawArgs,
      existing.postCommandArgs,
    );
    showModal(
      <ConfigEditorModal
        appId={appId}
        ...
        existingConfig={{
          ...
          profileName: '',
          protonVersion: report.protonVersion,
          launchOptions: prefilledLaunchOptions,
          enabledVars: existing.vars,
          appliedAt: 0,
          gpu: report.gpu || null,
          gpuDriver: report.gpuDriver || null,
          ram: report.ram || null,
          os: report.os || null,
